...
class PlayState extends MusicBeatState
{
	...
	var backgroundSkyline:FlxSprite;
	var midGroundTrees:FlxSprite;
	//var bushForeground:FlxSprite;
	var crowdLeft:FlxSprite;
	var crowdRight:FlxSprite;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'felt':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/felt/dialogue'));
			case 'noted':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/noted/dialogue'));
			case 'narc':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/narc/dialogue'));
			case 'god-complex':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/god-complex/dialogue'));
			case 'pico':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/pico/dialogue'));
		}
				...
				case 'park':
					{
						...
						var backgroundSkyline:FlxSprite = new FlxSprite(-460, -200).loadGraphic(Paths.image('avery/skyline'));
						backgroundSkyline.antialiasing = true;
						backgroundSkyline.setGraphicSize(Std.int(backgroundSkyline.width * 1.15));
						backgroundSkyline.updateHitbox();
						backgroundSkyline.scrollFactor.set(0.6, 0.9);
						backgroundSkyline.active = false;
						add(backgroundSkyline);

						var midGroundTrees:FlxSprite = new FlxSprite(-575, -200).loadGraphic(Paths.image('avery/midground'));
						midGroundTrees.antialiasing = true;
						midGroundTrees.setGraphicSize(Std.int(midGroundTrees.width * 1.25));
						midGroundTrees.updateHitbox();
						midGroundTrees.scrollFactor.set(0.9, 0.4);
						midGroundTrees.active = false;
						add(midGroundTrees);

						/*var bushForeground:FlxSprite = new FlxSprite(675, 425).loadGraphic(Paths.image('avery/bushforeground', 'shared'));
						bushForeground.antialiasing = true;
						bushForeground.setGraphicSize(Std.int(bushForeground.width * 1.25));
						bushForeground.updateHitbox();
						bushForeground.scrollFactor.set(0.7, 0.7);
						bushForeground.active = false;*/

						// sorry avery this fucking bush is a pain in my ass currently
						// nvm i think i fixed it

						crowdLeft = new FlxSprite(-735, 65);
						crowdLeft.frames = Paths.getSparrowAtlas('avery/crowd-bopper', 'shared');
						crowdLeft.animation.addByPrefix('bop', "Symbol 1", 24, false);
						//crowdLeft.setGraphicSize(Std.int(bushForeground.width * 1.1));
						crowdLeft.antialiasing = true;
						crowdLeft.scrollFactor.set(0.9, 0.9);
						crowdLeft.updateHitbox();
						add(crowdLeft);

						crowdRight = new FlxSprite(680, -100);
						crowdRight.frames = Paths.getSparrowAtlas('avery/crowdright', 'shared');
						crowdRight.animation.addByPrefix('bop', "Symbol 2 instance", 24, false);
						crowdRight.setGraphicSize(Std.int(crowdRight.width * 0.95));
						//crowdRight.setGraphicSize(Std.int(bushForeground.width * 1.1));
						crowdRight.antialiasing = true;
						crowdRight.scrollFactor.set(0.9, 0.33);
				...
				default:
					{
						...
						stageFront.scrollFactor.set(1.3, 0.9);
		...
		{
			case 'park':
				var bg:FlxSprite = new FlxSprite(-900, -50).loadGraphic(Paths.image('avery/bushforeground', 'shared'));			
				bg.antialiasing = true;
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			if (FlxG.save.data.distractions)
				{
					crowdLeft.animation.play('bop', true);
					crowdRight.animation.play('bop', true);
				}
		}
